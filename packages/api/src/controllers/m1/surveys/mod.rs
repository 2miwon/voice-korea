...
impl SurveyController {
    ...
    pub fn route(&self) -> Result<by_axum::axum::Router> {
        Ok(by_axum::axum::Router::new()
            .route("/", post(Self::update_survey_status))
            .with_state(self.clone()))
    }

    pub async fn update_survey_status(
        State(ctrl): State<SurveyController>,
        ...
        let progress_surveys = SurveyV2::query_builder()
            ...
            .project_type_equals(ProjectType::Survey)
            .query()
            .map(SurveyV2::from)
            .fetch_all(&ctrl.pool)
            .await?;

        tracing::info!("progress surveys: {:?}", progress_surveys);
        ...
        tracing::info!("finish surveys: {:?}", finish_surveys);
        ...
        for survey in progress_surveys {
                ...
                .await
            {
                Ok(_) => {
                    tracing::debug!(
                        "success to update survey status. (survey id: {:?})",
                        survey.id
                    );
                }
                Err(e) => {
                    tracing::error!(
                        "failed to update survey status with error: {:?} (survey id: {:?})",
                        e,
                        survey.id
                    );
                }
