...
use crate::{
    ...
    pages::deliberations::new::{
        ...
        details::votes::i18n::QuestionListTranslate,
    },
};
...
pub fn AddQuestion(lang: Language) -> Element {
    let tr: QuestionListTranslate = translate(&lang);
    rsx! {
        div { class: "cursor-pointer flex flex-col w-full h-200 rounded-lg justify-center items-center border border-dashed border-hint-gray mt-20",
...
pub fn QuestionTypeSelector(
    ...
    rsx! {
        select {
            class: "focus:outline-none w-215 h-55 justify-start items-start p-15 bg-background-gray rounded-sm mr-20 font-medium text-[15px] text-hint-gray",
            value: "{selected_type}",
